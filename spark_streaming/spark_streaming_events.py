...
sales_events_writer = create_write_stream_to_hive(sales_events,
                                                  checkpoint_path,
                                                  fact_sales)
product_review_events_writer = create_write_stream_to_hive(product_review_events,
                                                           checkpoint_path,
                                                           fact_product_review)
customer_interation_events_writer = create_write_stream_to_hive(customer_interation_events,
                                                                checkpoint_path,
                                                                fact_customer_interaction)

# START STREAMING (one time submit using spark-submit)
query1 = sales_events_writer.start()
query2 = product_review_events_writer.start()
query3 = customer_interation_events_writer.start()
